...
import OpenCV_secret_agents.image_classifier.ResizeUtils as ru
import OpenCV_secret_agents.image_classifier.WxUtils as wxutils
...
class InteractiveRecognizer(wx.Frame):
    ...
    def __init__(self, recognizerPath, cascadePath,
        ...
        size = ru.cvResizeCapture(self._capture, imageSize)
    ...
    def _detectAndRecognize(self, image):
        ...
        rects = self._detector.detectMultiScale( equalizedGrayImage, scaleFactor=self._scaleFactor, minNeighbors=self._minNeighbors,
        ...
        if len(rects) > 0:
            x, y, w, h = rects[0]
            self._currDetectedObject = cv2.equalizeHist(grayImage[y:y+h, x:x+w])
            if self._recognizerTrained:
                try:
                    labelAsInt, distance = self._recognizer.predict( self._currDetectedObject)
                    labelAsStr = BinasciiUtils.intToFourChars(labelAsInt)
                    self._showMessage( 'This looks most like %s.\n' 'The distance is %.0f.' % (labelAsStr, distance))
                except cv2.error:
    ...
    def _enableOrDisableUpdateModelButton(self):
        labelAsStr = self._referenceTextCtrl.GetValue()
        if len(labelAsStr) < 1 or self._currDetectedObject is None:
            self._updateModelButton.Disable()
        else:
